...
from logic import request_data, sync_customer_list, create_work_orders_list
...
def gui_audit_download(customer_name, start, end, filter_type):
    try:
        print(customer_name, start, end, filter_type)
        messagebox.showinfo(message="Download complete!")
    except Exception as e:
...
class DownloaderGUI(ctk.CTk):
    def __init__(self):
        ...
        self.customer_combobox = ctk.CTkComboBox(self.customer_frame, dropdown_font=('Arial', 12), values=[''], width=300)
        ...
        self.filter_dropdown.pack(pady=(4, 0), padx=(5, 20), side='right')
    ...
    def get_pm_audit_request(self):
        ...
        # customer_name param needs the full customer name from config.json lookup table
        gui_audit_download(customer_name=customer_list[customer],
                           start=start_date, end=end_date, filter_type=filter_type)
